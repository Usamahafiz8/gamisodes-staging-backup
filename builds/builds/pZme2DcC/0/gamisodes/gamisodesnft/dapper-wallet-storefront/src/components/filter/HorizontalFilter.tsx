...
const collections = [
  { label: "Genesis", value: ENftCollection.GENESIS, checked: false },
  { label: "Gadgets", value: ENftCollection.GADGETS, checked: false },
  { label: "Missions", value: ENftCollection.MISSIONS, checked: false },
  { label: "Brain Train", value: ENftCollection.BRAIN_TRAIN, checked: false },
  { label: "Gamisodes", value: ENftCollection.GAMISODES, checked: false },
  { label: "Moments", value: ENftCollection.MOMENTS, checked: false },
]
        ...
        >
          {collections.map(({ label, value }, index) => (
            <RadioGroup.Option
              ...
              href={value}
              key={index + label}
              className="cursor-pointer w-full lg:w-1/6 flex items-center justify-center truncate uppercase select-none font-semibold text-lg rounded-md"
              value={value}
            >
              {({ checked }) => (
                <span
