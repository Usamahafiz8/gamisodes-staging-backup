...
export interface IFilterState extends IFilterSingleCollection {
...
export function useCollectionFilter(selectedCollection) {
  ...
  useEffect(() => {
    const storageFilter = FilterStorage.get()
    if (storageFilter && storageFilter.collection === selectedCollection) {
      setFilter(storageFilter?.filter)
    } else { setFilter(allFilters[selectedCollection]) }
    setNfts(allCollections?.[selectedCollection])
